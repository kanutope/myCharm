...
OPENWEATHERMAP_KEY = '045d36dc1cfb3ade29817943a41cd3c3' # secret key 'pawawa+openweather@gmail.com'
DEPANNE = {'lan': 51.0935, 'lon': 2.5776}

licenseExpiryDate = datetime.datetime(2049,12,31, 0, 0)
LOXONE_WEATHER_SERVICE_PORT = 6066

# wget https://api.openweathermap.org/data/2.5/forecast?lat=51.0935&lon=2.5776&appid=${KEY}&units=metric&lang=nl

def downloadReport(latitude: object, longitude: object, asl: object) -> object:
    payload = {'units': 'metric', 'lang': 'nl'}

    r = requests.get('https://api.openweathermap.org/data/2.5/forecast?lat={lat:.4f}&lon={lon:.4f}&appid={key}'.format(lat=latitude, lon=longitude, key=OPENWEATHERMAP_KEY), params=payload)
    ...
    def do_GET(self):
        ...
        if path == '/forecast/':
            ...
            if os.path.isfile('weather.json'):
                ...
                jsonReport = json.loads(downloadReport(DEPANNE['lat'], DEPANNE['lon'], asl))
